...
# Corvus internal schema with descriptions
# LLM uses these to reason about what each field means
CORVUS_SCHEMA = {
...
def propose_mapping_with_llm(
    ...
    schema_description = "\n".join(
        f"  {field}: {desc}"
        for field, desc in CORVUS_SCHEMA.items()
    )

    sample_text = json.dumps(sample_rows, indent=2)

    prompt = f"""You are a data mapping assistant. 
Your job is to map columns from a customer's CSV file to the Corvus internal schema.

CORVUS SCHEMA (field: description):
{schema_description}
...
Instructions:
- For each Corvus field, identify the best matching CSV column
- Assign a confidence score between 0.0 and 1.0
- If no good match exists, set csv_column to null and confidence to 0.0
- Also identify any CSV columns that don't map to anything (unmapped)
- For status values found in the sample, propose a status_map 
  normalizing them to: In Progress, Blocked, Completed, Paused, Pending

Respond ONLY with valid JSON in this exact format, no other text:
{{
  "mapping": {{
    "build_id":           {{"csv_column": "...", "confidence": 0.0, "reasoning": "..."}},
    "station_id":         {{"csv_column": "...", "confidence": 0.0, "reasoning": "..."}},
    "operator_id":        {{"csv_column": "...", "confidence": 0.0, "reasoning": "..."}},
    "start_time":         {{"csv_column": "...", "confidence": 0.0, "reasoning": "..."}},
    "planned_end":        {{"csv_column": "...", "confidence": 0.0, "reasoning": "..."}},
    "needed_by_date":     {{"csv_column": "...", "confidence": 0.0, "reasoning": "..."}},
    "target_quantity":    {{"csv_column": "...", "confidence": 0.0, "reasoning": "..."}},
    "completed_quantity": {{"csv_column": "...", "confidence": 0.0, "reasoning": "..."}},
    "labor_hours":        {{"csv_column": "...", "confidence": 0.0, "reasoning": "..."}},
    "status":             {{"csv_column": "...", "confidence": 0.0, "reasoning": "..."}},
    "notes":              {{"csv_column": "...", "confidence": 0.0, "reasoning": "..."}}
  }},
  "status_map": {{
    "their_status_value": "Corvus normalized value"
  }},
...
def display_and_confirm_mapping(proposal: dict, headers: list[str]) -> dict:
    """
    ...
    if needs_review:
        ...
        for field, csv_col, confidence, reasoning in needs_review:
            ...
            if csv_col:
                user_input = input(
                    f"\n  Accept '{csv_col}'? Press Enter to accept, or type correct column name: "
                ).strip()
    ...
    if status_map:
        ...
        confirm = input("\n  Accept status mapping? (Enter=yes, n=skip): ").strip().lower()
        if confirm == "n":
            status_map = {}
...
def save_mapping_to_onboarding(
    ...
    """
    # clean None values from column_map
...
def run(csv_path: str, onboarding_path: str):
    """Main entry point."""
    ...
    if check_existing_mapping(onboarding, csv_path, fingerprint, headers):
        print("[CORVUS] Run 'python src/main.py' — mapping is ready.")
        return

    # propose mapping with LLM
...
if __name__ == "__main__":
    ...
    run(args.csv_path, args.onboarding)
